<template>
  <div>
    ...
    <p>
      按比例缩放图片。在保持图片原始宽高比的情况下缩放到指定的<code>newWidth</code>、<code>newHeight</code>大小。
    </p>
    <h4>
    ...
    <p>
      <code>beforeCallback</code>：前序遍历回调函数，回调参数为：root, parent,
      isRoot, layerIndex, index
    </p>
    <p>
      <code>afterCallback</code>：后序遍历回调函数，回调参数为：root, parent,
      isRoot, layerIndex, index
    </p>
  ...
  <span class="hljs-number">0</span>
    ...
    <p>
      <code>maxWidth</code>和<code>maxHeight</code>可以同时都传，也可以只传一个
    </p>
    <h4>resizeImg(imgUrl, maxWidth, maxHeight)</h4>
    <p>
      缩放图片，内部先加载图片，然后调用<code>resizeImgSize</code>方法，返回一个<code
        >promise</code
      >
    </p>
    ...
    <ul>
      <li>
        <p>
          <code>removeActiveState</code
          >：<code>Boolean</code>，默认为<code>false</code>，是否移除节点的激活状态
        </p>
      </li>
      <li>
        <p>
          <code>removeId</code>：v0.7.3-fix.1+，是否移除节点数据中的
          uid，默认为<code>true</code>
        </p>
      </li>
    </ul>
    <blockquote>
      <ul>
        <li>
          <code>keepId</code>：
          （原第四个参数）<code>Boolean</code>，默认为<code>false</code>，是否保留被复制节点的<code>id</code>，默认会删除<code>id</code>防止节点<code>id</code>重复，但是对于移动节点的场景，节点原<code>id</code>需要保留。
        </li>
      </ul>
    </blockquote>
    <p>
      复制节点树数据，主要是剔除其中的引用<code>node</code>实例的<code>_node</code>，然后复制<code>data</code>对象的数据，示例：
    </p>
  ...
  url, <span class="hljs-comment">// DataUrl</span>
    ...
    <p>
      将<code>&lt;p&gt;&lt;span&gt;&lt;/span&gt;&lt;p&gt;</code>形式的节点富文本内容转换成<code>\n</code>换行的文本。
    </p>
    ...
    <p>
      将<code>&lt;br&gt;</code>换行的文本转换成<code>&lt;p&gt;&lt;span&gt;&lt;/span&gt;&lt;p&gt;</code>形式的节点富文本内容。
    </p>
    <h4>isMobile()</h4>
    <blockquote>
      <p>v0.6.13+</p>
    ...
    <ul>
      ...
      <li>
        <p>
          <code>createNewId</code
          >：v0.7.3-fix.1+，<code>Boolean</code>，默认为<code>false</code>，即如果节点不存在<code>uid</code>的话，会创建新的<code>uid</code>。如果传<code>true</code>，那么无论节点数据原来是否存在<code>uid</code>，都会创建新的<code
            >uid</code
          >
        </p>
      </li>
...
&gt; -&gt; &amp;gt;
    ...
    <ul>
      <li>
        <code>a</code>、<code>b</code>：Object | Array, 要进行对比的两个对象
      </li>
    </ul>
  ...
  text, img
    ...
    <p>
      从节点的父节点的<code>nodeData.children</code>列表中移除该节点的数据。
    </p>
    ...
    <p>
      给 html 自闭合标签添加闭合状态，<code
        >&lt;div&gt;&lt;img src=&quot;xxx&quot;&gt;&lt;/div&gt;</code
      >
      -&gt;
    ...
    <p>
      获取当前浏览器使用的<code>Chrome</code>内核版本。如果当前浏览器使用的不是
      <code>Chrome</code>内核，那么会返回空字符串。
    </p>
