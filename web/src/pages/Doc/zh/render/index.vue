<template>
  <div>
    ...
    <p>
      <code>render</code
      >实例负载整个渲染过程，可通过<code>mindMap.renderer</code>获取到。
    </p>
    <h2>属性</h2>
    ...
    <p>
      高亮节点或子节点。如果<code>range</code>参数没有传递，那么直接高亮指定的<code>node</code>节点，如果<code>range</code>传递了一个要高亮的子节点的范围，那么会高亮该范围的子节点。
    </p>
    <p>
      高亮效果为通过一个矩形框来包裹，矩形的描边和填充样式可以通过<code>highlightNodeBoxStyle</code>实例化选项进行配置。
    </p>
    ...
    <ul>
      <li>
        <code>notRender</code
        >：v0.6.9+，<code>Boolean</code>，默认为<code>false</code>，是否不要触发渲染。
      </li>
    </ul>
    <p>
      设置节点数据，即<code>data</code>字段的数据，并会根据节点大小是否变化来判断是否需要重新渲染该节点，<code>data</code>为对象，如：<code
        >{text: '我是新文本'}</code
      >。
    </p>
    ...
    <p>
      复制节点，调用该方法后会存储当前激活的节点数据，后续调用<code>paste()</code>方法时可以进行粘贴。
    </p>
    <p>
      如果浏览器及协议（https）支持<code>js</code>操作剪贴板数据，那么复制的节点数据也会同时添加到用户的剪贴板中。
    </p>
    <h3>cut()</h3>
    <blockquote>
      <p>v0.6.8+</p>
    ...
    <p>
      剪切节点，调用该方法后会剪切当前激活的节点，并且存储该节点数据，后续调用<code>paste()</code>方法时可以进行粘贴。
    </p>
    <p>
      如果浏览器及协议（https）支持<code>js</code>操作剪贴板数据，那么复制的节点数据也会同时添加到用户的剪贴板中。
    </p>
    ...
    <p>
      粘贴节点，在调用了<code>copy()</code>或<code>cut()</code>方法后可以调用该方法进行粘贴节点。
    </p>
    <p>
      如果浏览器及协议（https）支持<code>js</code>操作剪贴板数据，那么其他地方复制的数据也可以进行粘贴，比如你可以进行跨浏览器粘贴<code>simple-mind</code>节点，如果是非<code>simple-mind</code>节点数据，那么会提取出剪切板中的文本和图片进行粘贴，文本默认会粘贴为子节点，图片默认会添加到当前的节点中。
    </p>
    <h3>clearCache()</h3>
    <blockquote>
      <p>v0.9.2+</p>
    ...
    <p>清空节点缓存池。</p>
    <h3>
      emitNodeActiveEvent(node = null, activeNodeList =
      [...this.activeNodeList])
    </h3>
    <ul>
      <li>
