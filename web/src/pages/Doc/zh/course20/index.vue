<template>
  <div>
    ...
    <p>
      如果你想自定义节点的内容，那么可以在实例化<code>simple-mind</code>时传入以下选项：
    </p>
    ...
    <p>
      <code>customCreateNodeContent</code
      >方法会接收当前遍历到的节点实例作为参数，一般而言你会需要该节点的数据，这可以通过如下方式获取：
    </p>
    ...
    <p>
      <code>customCreateNodeContent</code
      >方法需要返回<code>DOM</code>节点，如果某个节点你不想自定义，那么可以返回<code>null</code>，那么还是会走内置的节点渲染逻辑。
    </p>
    <p>
      返回的<code>DOM</code>节点的宽高需要是确定的，如果是动态的那么会导致宽高获取错误，最终导致节点定位错误和发生重叠等问题。
    </p>
    <p>
    ...
    <h2>示例2：渲染Vue2组件</h2>
    <p>
      如果想要使用一个相对简单的<code>Vue</code>组件，那么可以通过如下方式：
    </p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> CustomNodeContent <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;CustomNodeContent.vue&#x27;</span>
...

{
    ...
    <p>
      如果你的<code>Vue</code>组件比较复杂，里面用到了<code>vueRouter</code>、<code>vuex</code>、<code>i18n</code>等，那么要和你项目的入口组件一样，在实例化时要把这些内容也加载到组件内，不然会报错。
    </p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> CustomNodeContent <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;CustomNodeContent.vue&#x27;</span>
...

{
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> { createApp } <span class="hljs-keyword">from</span> <span class="hljs-string">&quot;vue&quot;</span>
...

{
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> { createRoot } <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;react-dom/client&#x27;</span>

{
    ...
    <blockquote>
      <p>
        感谢<a href="https://github.com/h5chenhang">h5chenhang</a>贡献的<a
          href="https://github.com/wanglin2/mind-map/issues/192"
          >示例代码</a
        >。
      </p>
    </blockquote>
