<template>
  <div>
    ...
    <p>
      该插件的原理是使用<a href="https://github.com/quilljs/quill">Quill</a
      >编辑器实现富文本编辑，然后把编辑后生成的<code>DOM</code>节点直接作为节点的文本数据，并且在渲染的时候通过<code>svg</code>的<code>foreignObject</code>标签嵌入<code>DOM</code>节点。
    </p>
    <blockquote>
      <p>v0.5.6即以前的版本存在以下提示：</p>
      <p>
        这样也造成了一个问题，就是导出为图片的功能受到了影响，原本将<code>svg</code>导出为图片的原理很简单，获取到<code>svg</code>字符串，然后创建为<code>type=image/svg+xml</code>类型的<code>blob</code>数据，再使用<code>URL.createObjectURL</code>方法生成<code>data:url</code>数据，再创建一个<code>Image</code>标签，将<code>data:url</code>作为该图片的<code>src</code>，最后再将这个图片绘制到<code>canvas</code>对象上进行导出，但是经过测试，当<code>svg</code>中嵌入了<code>DOM</code>节点，这种方式导出会出错，并且尝试了多种方式后都无法实现完美的导出效果，目前的方式是遍历<code>svg</code>中的<code>foreignObject</code>节点，使用<a
          href="https://github.com/niklasvh/html2canvas"
          >html2canvas</a
        >将<code>foreignObject</code>节点内的<code>DOM</code>节点转换为图片再替换掉<code>foreignObject</code>节点，这种方式可以工作，但是非常耗时，因为<code>html2canvas</code>转换一次的时间很长，导致转换一个节点都需要耗时差不多2秒，这样导致节点越多，转换时间越慢，所以如果无法忍受长时间的导出的话推荐不要使用该插件。
      </p>
    </blockquote>
    <blockquote>
      <p>
        <code>v0.5.7+</code
        >的版本直接使用<code>html2canvas</code>转换整个<code>svg</code>，速度不再是问题，但是目前存在一个<code>bug</code>，就是节点的颜色导出后不生效。
      </p>
    </blockquote>
    <p>
      <code>v0.6.13+</code>版本使用<a
        href="https://github.com/1904labs/dom-to-image-more"
        >dom-to-image-more</a
      >替换了<code>html2canvas</code>，解决了节点的颜色导出后不生效的问题。
    </p>
    <blockquote>
      <p>
    ...
    <p>
      从<code>0.6.16+</code>版本后不再使用<code>dom-to-image-more</code>、<code>html2canvas</code>之类的第三方库实现导出，兼容性及导出都不再有问题。
    </p>
    <h2>注册</h2>
    <pre
      class="hljs"
...
<span class="hljs-comment">// import RichText from &#x27;simple-mind/src/RichText.js&#x27; v0.6.0以下版本使用该路径</span>

MindMap.usePlugin(RichText, opt?)
</code></pre>
    <p>
      注册完且实例化<code>MindMap</code>后可通过<code>mindMap.richText</code>获取到该实例。
    </p>
    <h3>注册选项</h3>
  ...
  <span class="hljs-string">&#x27;serif&#x27;</span>
    ...
    <ul>
      <li>
        <code>config</code>：对象，键为样式属性，值为样式值，完整的配置如下：
      </li>
    </ul>
    <pre class="hljs"><code>{
    ...
    <ul>
      ...
      <li>
        <p>
          <code>pure</code
          >：v0.9.4+，如果设为true，那么仅会格式化文本样式，不会同步到节点的数据中
        </p>
      </li>
    </ul>
...

{
    ...
    <p>
      将<code>svg</code>中嵌入的<code>dom</code>元素转换成图片，返回一个<code>Promise</code>。
    </p>
