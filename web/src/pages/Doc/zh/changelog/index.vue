<template>
  <div>
    ...
    <blockquote>
      ...
      <p>
        4.修复 mac 上 useLeftKeySelectionRightKeyDrag 选项设为 true 时无法按住
        ctrl 键多选节点的问题。
      </p>
      <p>
    ...
    <blockquote>
      ...
      <p>
        3.优化节点存在超链接、备注时在 Firefox
        浏览器中导出图片的宽高不正确的问题。
      </p>
      <p>
        4.节点文本编辑中，如果粘贴的是 smm
        格式的粘贴数据，那么会取出第一个节点的纯文本进行粘贴。
      </p>
    ...
    <blockquote>
      ...
      <p>
        6.修复滚动条插件在 initRootNodePosition
        配置不为默认的[center,center]时位置计算错误的问题。
      </p>
      <p>7.修复拖拽画布时鼠标在节点上面松开时拖拽无法停止的问题。</p>
      <p>
        8.修复当画布容器距浏览器窗口左上角不为 0 时，view.fit
        方法计算出来的位置有误差的问题。
      </p>
      <p>
      ...
      <p>
        12.修复节点富文本编辑时，文本选中范围为 null 时没有触发
        rich_text_selection_change 事件的问题。
      </p>
    ...
    <blockquote>
      <p>
        1.修改 mousewheel 事件,dir 标志修改为
        dirs,支持存储多个方向,优化触控板的双指移动操作。
      </p>
      ...
      <p>
        9.重构 pdf 的导出逻辑，导出的 pdf 尺寸不再是固定的
        a4，而是思维导图的尺寸，同时删除分页导出的配置。
      </p>
    ...
    <blockquote>
      <p>
        1.修复历史记录数据中概要节点的激活状态未被删除的问题，会导致点击概要节点时触发
        data_change 事件。
      </p>
    ...
    <blockquote>
      ...
      <p>
        3.导出 png 的方法新增压缩参数；优化大数据量节点导出 pdf
        时体积过大的问题。
      </p>
      <p>
    ...
    <h2>0.8.0-fix.1</h2>
    <p>
      修复：修复直接粘贴的方式创建新节点时如果粘贴的内容带有&lt;&gt;等 html
      标签符号时新创建的节点内容为空的问题。
    </p>
    <h2>0.8.0</h2>
    <p>
      破坏性更新：大幅优化部分代码，小幅提升性能，主要是<code>render</code>类，删除无用逻辑、调整不合理的实现、提取重复代码；修改函数名称、函数功能等。
    </p>
    <p>修复：</p>
    <blockquote>
      ...
      <p>
        3.修复插入概要、上移、下移、一键整理布局的快捷键操作没有触发 data_change
        事件的问题。
      </p>
    ...
    <h2>0.7.0</h2>
    <p>
      破坏性更新：删除了主题文件中节点激活样式的部分，不再支持设置节点的激活样式，激活效果改为统一的节点外边框样式，同时支持鼠标
      hover 效果。
    </p>
    <p>修复：</p>
    <blockquote>
    ...
    <blockquote>
      ...
      <p>6.默认关闭双击复位画布。</p>
      <p>
        7.修改导出图片方法的参数,导出 pdf 时如果思维导图尺寸小于 a4
        纸那么不旋转方向。
      </p>
    ...
    <blockquote>
      ...
      <p>
        2.优化导出图片逻辑，遍历节点转换图片的 url 时，如果已经是 data:URL
        形式不重复处理。
      </p>
    </blockquote>
    <p>新增：</p>
    ...
    <blockquote>
      <p>
        1.去除创建隐藏输入框，通过 navigator.clipboard
        实现复制粘贴；支持跨浏览器粘贴思维导图节点数据；支持自定义处理剪贴板中的文本数据。
      </p>
    </blockquote>
    <p>Demo：</p>
    <blockquote>
    ...
    <blockquote>
      ...
      <p>
        3.导出图片由 html2canvas 库改为使用 dom-to-image-more
        库，解决导出富文本节点文字样式丢失的问题。
      </p>
    ...
    <blockquote>
      <p>
        1.节点移动结束事件（node_dragend）增加回调参数，可以获取到移动到节点的
        uid。
      </p>
    ...
    <p>
      Demo：1.新增主题、主题列表新增 tab 区分。
      2.节点图片上传支持输入网络图片地址。 3.节点图片上传支持输入网络图片。
    </p>
    <h2>0.6.10</h2>
    <p>
      修复：1.修复搜索定位到某个节点后删除该节点，再次搜索时搜索结果未更新的问题。
      2.修复调整图片大小的按钮在节点操作后没有更新的问题。
      3.修复内部数据深拷贝位置不正确的问题。 4.修复富文本节点换行不生效的问题。
      5.修复切换主题等场景时节点换行会丢失的问题。
    </p>
    <p>新增：1.搜索支持搜索空白字符和替换为空白字符。</p>
    <p>
      Demo：1.支持通过图标按钮调出搜索。 2.支持通过图标按钮切换暗黑模式。
      3.优化搜索:鼠标不在搜索区域内不聚焦,解决鼠标不在搜索区域内无法删除输入的文字的问题。
      4.调整添加节点图标的界面 UI，新增系列节点图标。 5.新增贴纸列表。
      6.修复在搜索框回车后输入框焦点丢失的问题。
      7.支持点击节点内的图标显示一个图标快捷替换和删除悬浮面板。
    </p>
    <h2>0.6.9-fix.1</h2>
    ...
    <h2>0.6.9</h2>
    <p>
      修复：1.修复给概要节点设置样式概要节点会消失的问题。2.修复自定义节点内容时，二次创建根实例时节点内容不渲染的问题。3.修复节点处于编辑中时添加新节点时新节点的焦点丢失问题。
      2.修复连续按 tab 键无法连续创建子节点的问题。
    </p>
    <p>
      新增：1.导出 svg 时替换 svg
      中存在的<code>&amp;nbsp;</code>字符，避免导出的 svg 报错。
      2.支持搜索和替换。
    </p>
    <p>Demo：1.切换主题时支持选择是否覆盖设置过的基础样式。</p>
    <h2>0.6.8</h2>
    <p>
      修复：1.修改插入概要的快捷键为 Ctrl+G，避免和保存快捷键冲突。
      2.修复节点正在编辑时切换富文本编辑配置输入框出现异常的问题。
    </p>
    <p>新增：1.修改复制、剪切、粘贴逻辑，支持粘贴剪切板中的数据。</p>
    <p>
      Demo：1.修复基础样式-设置节点外边距未保存的问题。
      2.支持根据主题自动切换为暗黑模式。
    </p>
    <h2>0.6.7</h2>
    <p>
      修复：1.修复节点收起再展开后展开收起按钮占位元素丢失的问题。
      2.修复只读模式下可以缩放图片的问题。
    </p>
    <p>
      新增：1.支持根据节点实例或节点 uid 定位到某个节点。 2.修改节点 uid
      的创建方式，导出数据添加节点的 uid。
    </p>
    <p>移除：1.移除节点过渡效果。</p>
    <p>
      Demo：1.添加网站首页。 2.修复大纲里创建新节点时节点样式丢失的问题。
      3.修复大纲里编辑节点后按回车或 Tab 键后编辑文本丢失的问题。
      4.优化大纲的节点定位，被收起的节点会自动展开。 5.侧边栏按钮支持收起。
      6.优化小屏适配。
    </p>
    <h2>0.6.6</h2>
    <p>
      新增：1.支持导出为 Xmind 新版文件。2.导入 Xmind
      新版文件支持导入节点中的图片。 3.新增竖向时间轴结构。
    </p>
    <p>
      修复：1.TouchEvent 插件不再派发 click
      事件，解决移动端点击超链接会打开两个窗口的问题。
      2.修复拖拽移动一个节点成为另一个节点的子节点时该节点的父节点指向未更新的问题。
      3.修复二级节点拖拽成三级节点时节点边框样式未更新的问题。
      4.修复向右生长的结构外其他结构鼠标移入展开收起按钮位置时不会触发按钮显示的问题。
    </p>
    <p>
    ...
    <h2>0.6.5</h2>
    <p>
      修复：1.修复 xmind 文件导入报错的问题。
      2.修复极少数情况下当节点文本的宽度为小数时显示发生换行的问题。
    </p>
    <p>
      新增：1.打包后的库支持获取内置常量、主题等数据。
      2.支持配置鼠标滚轮方向对应的缩放行为。 3.节点图片支持拖拽调整大小。
    </p>
    <h2>0.6.4-fix.1</h2>
    <p>
    ...
    <h2>0.6.0</h2>
    <p>
      破坏性更新：调整了 simple-mind
      源码的目录结构，主要影响：1.插件的引入路径需要修改。2.constant
      文件路径需要修改。
    </p>
    <p>
      新增：1.支持一键缩放至适应画布功能。 2.按住 Ctrl
      键多选功能可通过配置按需开启。 3.支持设置为左键多选节点，右键拖动画布。
      4.支持控制节点是否允许编辑。 5.新增销毁思维导图的方法。
      6.新增触摸事件支持插件。
    </p>
    <p>
      修复：1.修复按住 ctrl 键多选节点时不会触发节点的 click 事件的问题。
      2.修复清空一个节点后再输入文字时节点样式丢失的问题。
    </p>
    <h2>0.5.11</h2>
    <p>新增：支持关联性文本编辑。</p>
    ...
    <h2>0.5.9</h2>
    <p>
      修改：统一导出方法的格式，使用<code>FileReader</code>代替<code>URL.createObjectURL</code>转换<code>blob</code>数据。
    </p>
    <h2>0.5.8</h2>
    <p>
      优化：1.节点位置没有变化不触发位置设置。
      2.展开收起状态没有变化不触发按钮更新。
    </p>
    <p>
      新增：1.默认改为鼠标移上节点才显示展开收起按钮。
      2.支持扩展节点可插入的图标列表。
    </p>
    <h2>0.5.7</h2>
    <p>
      破坏性更新：富文本模式下导出 png 改为使用 html2canvas 转换整个
      svg，大幅提高导出速度，不过 html2canvas 存在一个 bug，foreignObject
      元素中的 dom
      节点内联的文字颜色无法识别，所以导出节点的文字颜色是固定的，不过相对于之前的导出基本不可用状态，目前至少能快速顺利的导出。
    </p>
    <p>优化：优化富文本节点编辑体验。</p>
    <p>
    ...
    <h2>0.5.6</h2>
    <p>
      修复：1.修复短时间快速多次渲染时节点位置错乱的问题。
      2.修复节点正在编辑中时拖动画布导致编辑框和节点分离的问题。
    </p>
    <p>新增：1.添加最大历史记录数限制。</p>
    <h2>0.5.5</h2>
    <p>
      新增：1.支持配置导出为 png、svg、pdf 时的内边距。
      2.支持配置节点文本编辑框、节点备注浮层元素的 z-index。
      3.支持点击画布外的区域结束节点编辑状态。
    </p>
    <h2>0.5.5-fix.1</h2>
    <p>
      修复：1.修复节点在画布外编辑时编辑框也在画布外的问题。
      2.修改结构后复位变换，防止存在缩放时切换结构后第一次拖动时会发生位置突变的问题。
    </p>
    ...
    <p>
      修复：1.修复节点右键和画布右键的冲突问题。
      2.修复组织结构图、目录组织图等节点拖拽时存在线段未隐藏的 bug。
    </p>
    ...
    <h2>0.5.2</h2>
    <p>
      修复：1.导出的<code>json</code>数据中去除<code>uid</code>；2.重新渲染时清空节点缓存池。
    </p>
    <h2>0.5.1</h2>
 ...
 5.优化节点编辑的体验；
    ...
    <p>
      修改：从 0.5.0
      版本开始，考虑性能问题，节点激活状态只能修改形状相关的样式：
    </p>
    <pre
      class="hljs"
    ><code>;[<span class="hljs-string">&#x27;fillColor&#x27;</span>, <span class="hljs-string">&#x27;borderColor&#x27;</span>, <span class="hljs-string">&#x27;borderWidth&#x27;</span>, <span class="hljs-string">&#x27;borderDasharray&#x27;</span>, <span class="hljs-string">&#x27;borderRadius&#x27;</span>]
</code></pre>
    <h2>0.4.7</h2>
    <p>
      优化：1.富文本编辑时初始聚焦时不再默认全选；2.富文本编辑时使用节点填充色作为背景色，避免节点颜色为白色时看不见。
      3.节点激活状态切换不再触发历史记录。
      4.短时间多次触发历史记录，只会添加最后一次的数据。
      5.优化历史记录添加，当有回退时，再次添加新记录时删除当前指针后面的历史数据。
    </p>
    <p>
      新增：1.支持导入和导出 Markdown 格式文件。
      2.支持配置插入节点时的初始文字。
      3.扩展插入节点和删除节点的命令，支持指定节点。
    </p>
    <h2>0.4.6</h2>
    <p>新增：1.关联线支持调整控制点。</p>
    <p>优化：1.添加历史数据时过滤和上一次相比没有改变的数据。</p>
    <p>
      修复：1.修复节点编辑时方向键和方向键导航功能的冲突问题。
      2.修复拖拽移动节点时节点 id 的丢失问题，这会导致关联线丢失。
    </p>
    <h2>0.4.5</h2>
    <p>
      新增：1.支持关联线。 2.按住根节点也可以拖动画布。3.按住 ctrl
      键可以调整多选节点。
    </p>
    <h2>0.4.4</h2>
    <p>新增：支持响应鼠标的横向滚动。</p>
    ...
    <h2>0.4.2</h2>
    <p>
      新增：<code>Node</code>类的<code>setText</code>方法增加第二个参数，以支持设置富文本内容。
    </p>
    <h2>0.4.1</h2>
    <p>
    ...
    <p>
      修复：1.Mac 系统触控板缩放相反的问题；2.设备 window.devicePixelRatio 不为
      1 时，当存在富文本节点时导出的图片中富文本节点尺寸会变大的问题。
    </p>
    <h2>0.4.0</h2>
    <p>新增：节点支持富文本编辑。</p>
    <h2>0.3.4</h2>
    ...
    <h2>0.2.24</h2>
    <p>
      新增：节点自由拖拽改为可配置，默认为<code>false</code>不开启；支持添加水印。
    </p>
    <h2>0.2.23</h2>
    <p>新增：支持注册新主题。</p>
    <h2>0.2.22</h2>
    <p>
      优化：取消内置<code>simple-mind</code>包的主题和结构图片，改为由使用者自行维护，原有图片可在<code>web/assets/img/</code>目录找到。
    </p>
    <h2>0.2.21</h2>
    <p>新增：支持节点横线风格。</p>
    <h2>0.2.20</h2>
    ...
    <p>
      新增：经典 4
      主题；支持添加概要；支持自由拖拽；上移节点、下移节点、复制节点、剪切节点、粘贴节点、一键整理布快捷键；库打包；Ctrl+左键多选。
    </p>
    <h2>0.1.18</h2>
