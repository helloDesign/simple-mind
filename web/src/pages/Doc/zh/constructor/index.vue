...
<span class="hljs-keyword">const</span> mindMap = <span class="hljs-keyword">new</span> MindMap({
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>null</td>
          <td>
            自定义节点备注内容显示，Object类型，结构为：{show: (noteContent,
            left, top, node) =&gt; {//
            你的显示节点备注逻辑。node为v0.8.1+版本新增的回参，代表节点实例 },
            hide: () =&gt; {// 你的隐藏节点备注逻辑 }}
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>
            鼠标滚轮的行为，<code>zoom</code>（放大缩小）、<code>move</code>（上下移动）。如果<code>customHandleMousewheel</code>传了自定义函数，这个属性不生效
          </td>
        </tr>
        <tr>
          ...
          <td>100</td>
          <td>
            当<code>mousewheelAction</code>设为<code>move</code>时，可以通过该属性控制鼠标滚动一下视图移动的步长，单位<code
              >px</code
            >
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>null</td>
          <td>
            初始根节点的位置，可传一个数组，默认为<code
              >['center', 'center']</code
            >，代表根节点处于画布中心位置，除了<code>center</code>，关键词还可以设置<code>left</code>、<code>top</code>、<code>right</code>、<code>bottom</code>，除了可以传关键词，数组的每项还可以传递一个数字，代表具体的像素，可以传递一个百分比字符串，比如<code
              >['40%', '60%']</code
            >，代表水平位置在画布宽度的<code>40%</code>的位置，垂直位置在画布高度的<code>60%</code>的位置
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>null</td>
          <td>
            自定义对剪贴板文本的处理。当按ctrl+v粘贴时会读取用户剪贴板中的文本和图片，默认只会判断文本是否是普通文本和simple-mind格式的节点数据，如果你想处理其他思维导图的数据，比如processon、zhixi等，那么可以传递一个函数，接受当前剪贴板中的文本为参数，返回处理后的数据，可以返回两种类型：1.返回一个纯文本，那么会直接以该文本创建一个子节点；2.返回一个节点对象，格式如下：{
            simpleMindMap: true, data: { data: { text: '' }, children: [] }
            }，代表是simple-mind格式的数据，节点数据同simple-mind节点数据格式，如果你的处理逻辑存在异步逻辑，也可以返回一个promise
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>
            拖拽多个节点时随鼠标移动的示意矩形的样式配置，传递一个对象，字段含义分别为矩形的宽、高、填充色
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>
            节点拖拽时的透明度配置，传递一个对象，字段含义分别为：跟随鼠标移动的克隆节点或矩形的透明度、被拖拽节点的透明度
          </td>
        </tr>
        <tr>
          ...
          <td>{}</td>
          <td>
            自定义节点标签的颜色，可传一个对象，key为要指定颜色的标签内容，value为该标签内容的颜色，如果不传内部会根据标签内容生成对应的颜色
          </td>
        </tr>
        <tr>
          ...
          <td>
            节点协作编辑时的人员头像样式配置，字段含义分别为：头像大小、如果是文字头像，那么文字的大小
          </td>
        </tr>
        <tr>
          ...
          <td>true</td>
          <td>
            关联线是否始终显示在节点上层，如果设为false，那么创建关联线和激活关联线时处于最顶层，其他情况下处于节点下方
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>null</td>
          <td>
            粘贴文本的方式创建新节点时，控制是否按换行自动分割节点，即如果存在换行，那么会根据换行创建多个节点，否则只会创建一个节点，可以传递一个函数，返回promise，resolve代表根据换行分割，reject代表忽略换行
          </td>
        </tr>
        <tr>
          ...
          <td>100</td>
          <td>
            指定时间内只允许添加一次历史记录，避免添加没有必要的中间状态，单位：ms
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>null</td>
          <td>
            在节点上粘贴剪贴板中的图片的处理方法，默认是转换为data:url数据插入到节点中，你可以通过该方法来将图片数据上传到服务器，实现保存图片的url。可以传递一个异步方法，接收Blob类型的图片数据，需要返回指定结构：{
        ...
        <tr>
          ...
          <td>false</td>
          <td>
            是否将思维导图限制在画布内。比如向右拖动时，思维导图图形的最左侧到达画布中心时将无法继续向右拖动，其他同理
          </td>
        </tr>
        <tr>
          ...
          <td>true</td>
          <td>
            当注册了滚动条插件（Scrollbar）时，是否将思维导图限制在画布内，isLimitMindMapInCanvas配置不再起作用
          </td>
        </tr>
        <tr>
          ...
          <td>{ from: '', to: '' }</td>
          <td>
            默认情况下，新创建的关联线两个端点的位置是根据两个节点中心点的相对位置来计算的，如果你想固定位置，可以通过这个选项来配置。from和to都不传，则都自动计算，如果只传一个，另一个则会自动计算。from和to可选值：left、top、bottom、right
          </td>
        </tr>
        <tr>
  ...
  children [<span class="hljs-comment">// 子节点，结构和根节点一致</span>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td></td>
          <td>
            分组下的图标列表，数组的每一项为一个对象，<code
              >{ name: '', icon: '' }</code
            >，<code>name</code>代表图标的名称，<code>icon</code>代表图标，可以是<code>svg</code>图标，比如<code
              >&lt;svg ...&gt;&lt;path&gt;&lt;/path&gt;&lt;/svg&gt;</code
            >，也可以是图片<code>url</code>，或者是<code>base64</code>图标，比如<code
    ...
    <p>
      ...
      >内置了众多主题，另外你也可以注册新主题，建议在实例化之前进行注册，这样在实例化时可以直接使用新注册的主题，使用示例：
    </p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> MindMap <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;simple-mind&#x27;</span>
...
mindMap.setTheme(<span class="hljs-string">&#x27;主题名称&#x27;</span>)
    ...
    <ul>
      <li>
        <code>opt</code
        >：v0.4.0+，插件参数。如果某个插件支持自定义选项的话可以通过这个参数传入。
      </li>
    </ul>
    <p>
    ...
    <p>
      获取是否注册了某个插件，返回的是插件在注册插件列表里的索引，为<code>-1</code>则代表插件没有注册。
    </p>
    <h2>静态属性</h2>
    <h3>pluginList</h3>
    <blockquote>
    ...
    <h3>elRect</h3>
    <p>
      容器元素<code>el</code>的尺寸、位置信息。调用<code>getBoundingClientRect()</code>方法的返回结果。
    </p>
    <h3>width</h3>
    ...
    <p>
      清空<code>lineDraw</code>、<code>associativeLineDraw</code>、<code>nodeDraw</code>、<code>otherDraw</code>容器。
    </p>
    <h3>destroy()</h3>
    <blockquote>
      <p>v0.6.0+</p>
    ...
    <p>
      <code>ignoreWatermark</code
      >：v0.8.0+，不要绘制水印，如果不需要绘制水印的场景可以传<code>true</code>，因为绘制水印非常慢
    </p>
  ...
  scaleY, <span class="hljs-comment">// Number，思维导图图形的垂直缩放值</span>
    ...
    <ul>
      <li>
        <code>callback</code
        >：<code>v0.3.2+</code>，<code>Function</code>，当重新渲染完成时调用
      </li>
    </ul>
    <p>
      触发整体渲染，会进行节点复用，性能较<code>reRender</code>会更好一点，如果只是节点位置变化了可以调用该方法进行渲染
    </p>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>鼠标滚动事件</td>
          <td>
            e（事件对象）、dir（向上up还是向下down滚动。v0.9.2+已改为dirs，数组类型，即支持同时保存多个方向）、this（Event事件类实例）、isTouchPad（v0.6.1+，是否是触控板触发的事件）
          </td>
        </tr>
        <tr>
          <td>contextmenu</td>
        ...
        <tr>
          ...
          <td>
            当注册了<code>RichText</code>插件时可用。当节点编辑时，文本选区发生改变时触发
          </td>
          <td>
        ...
        <tr>
          <td>resize（v0.8.0+）</td>
          <td>
            容器尺寸改变后触发，实际上是当思维导图实例的<code>resize</code>方法被调用后触发
          </td>
          <td></td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>
            渲染树数据变化的明细，会返回一个数组，每一项代表一个更新点，每一项都是一个对象，存在一个<code>type</code>属性，代表明细的类型，包含<code>create</code>（创建节点）、<code>update</code>（更新节点）、<code>delete</code>（删除节点），存在一个<code>data</code>属性，代表当前更新的节点数据，如果是<code>update</code>类型，还会存在一个<code>oldData</code>属性，保存了更新前该节点的数据
          </td>
          <td>arr（明细数据）</td>
        </tr>
        <tr>
    ...
    <p>
      设置主题配置，<code>config</code>同上面选项表格里的选项<code
        >themeConfig</code
      >
    </p>
    ...
    <p>
      更新配置，即更新<code>new MindMap(opt)</code
      >的<code>opt</code>，可以只更新部分数据，比如：
    </p>
    ...
    <table>
      ...
      <tbody>
        ...
        <tr>
          <td>INSERT_NODE</td>
          <td>
            插入同级节点，操作节点为当前激活的节点或指定节点，如果有多个激活节点，只会对第一个有效（v0.7.2+支持对多个激活节点同时插入兄弟节点）
          </td>
          <td>
            openEdit（v0.4.6+，是否激活新插入的节点并进入编辑模式，默认为<code>true</code>）、
            appointNodes（v0.4.7+，可选，指定要插入兄弟节点的节点，指定多个节点可以传一个数组）、
            appointData（可选，指定新创建节点的数据，比如{text: 'xxx',
            ...}，详细结构可以参考<a
        ...
        <tr>
          ...
          <td>
            data（要粘贴的节点数据，一般通过<code>renderer.copyNode()</code>方法和<code>renderer.cutNode()</code>方法获取）
          </td>
        </tr>
        <tr>
          <td>CUT_NODE</td>
        ...
        <tr>
          <td>SET_NODE_DATA</td>
          <td>
            更新节点数据，即更新节点数据对象里<code>data</code>对象的数据，注意这个命令不会触发视图的更新
          </td>
          <td>
            node（要设置的节点）、data（对象，要更新的数据，如<code
              >{expand: true}</code
            >）
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>设置节点图片</td>
          <td>
            node（要设置的节点）、imgData（对象，图片信息，结构为：<code
              >{url, title, width, height}</code
            >，图片的宽高必须要传）
          </td>
        </tr>
        <tr>
          ...
          <td>设置节点图标</td>
          <td>
            node（要设置的节点）、icons（数组，预定义的图片名称组成的数组，可用图标可在<a
              href="https://github.com/wanglin2/mind-map/blob/main/simple-mind/src/svg/icons.js"
              >icons.js</a
            >文件里的<code>nodeIconList</code>列表里获取到，图标名称为<code>type_name</code>，如<code>['priority_1']</code>）
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>设置节点标签</td>
          <td>
            node（要设置的节点）、tag（字符串数组，内置颜色信息可在<a
        ...
        <tr>
          ...
          <td>
            node（要移动的节点，（v0.7.2+支持传递节点数组实现同时移动多个节点））、
            exist（目标节点）
          </td>
        </tr>
        <tr>
          ...
          <td>
            将节点移动到另一个节点的前面，（v0.7.2+支持传递节点数组实现同时移动多个节点）
          </td>
        ...
        <tr>
          ...
          <td>
            移动节点作为另一个节点的子节点，（v0.7.2+支持传递节点数组实现同时移动多个节点）
          </td>
        ...
        <tr>
          ...
          <td>添加节点概要</td>
          <td>
            data（概要的数据，对象格式，节点的数字段都支持，默认为{text:
            '概要'}）
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>设置节点自定义位置</td>
          <td>
            node（要设置的节点）、 left（自定义的x坐标，默认为undefined）、
            top（自定义的y坐标，默认为undefined）
          </td>
        </tr>
        <tr>
        ...
        <tr>
          ...
          <td>
            给指定的节点同时插入多个同级节点，操作节点为当前激活的节点或指定节点
          </td>
          <td>
            appointNodes（可选，指定节点，指定多个节点可以传一个数组）,
            nodeList（新插入节点的数据列表，数组类型）
          </td>
        </tr>
        <tr>
          <td>INSERT_MULTI_CHILD_NODE（v0.7.2+）</td>
          <td>
            给指定的节点同时插入多个子节点，操作节点为当前激活的节点或指定节点
          </td>
          <td>
            appointNodes（可选，指定节点，指定多个节点可以传一个数组）,
            childList（新插入节点的数据列表，数组类型）
          </td>
        </tr>
        <tr>
          ...
          <td>
            formula（要插入的数学公式，LaTeX 语法）,
            appointNodes（可选，指定要插入公式的节点，多个节点可以传数组，否则默认为当前激活的节点）
          </td>
        </tr>
        <tr>
          ...
          <td>
            openEdit（是否激活新插入的节点并进入编辑模式，默认为<code>true</code>）、
            appointNodes（可选，指定要插入父节点的节点，指定多个节点可以传一个数组）、
            appointData（可选，指定新创建节点的数据，比如{text: 'xxx',
            ...}，详细结构可以参考<a
              href="https://github.com/wanglin2/mind-map/blob/main/simple-mind/example/exampleData.js"
              >exampleData.js</a
        ...
        <tr>
          ...
          <td>
            appointNodes（可选，指定要删除的节点，指定多个节点可以传一个数组）
          </td>
        </tr>
        <tr>
          ...
          <td>将指定节点上移一个层级</td>
          <td>
            node（可选，指定要上移层级的节点，不传则为当前激活节点中的第一个）
          </td>
        </tr>
        <tr>
          ...
          <td>
            node（可选，指定要清除自定义样式的节点，不传则为当前激活节点中的第一个）
          </td>
        </tr>
        <tr>
          ...
          <td>
            appointNodes（可选，节点实例数组，指定要去除自定义样式的多个节点，如果不传则会去除当前画布所有节点的自定义样式）
          </td>
        </tr>
      </tbody>
    </table>
    ...
    <p>
      ...
      >：<code>Boolean</code>，默认为<code>false</code>，即获取的数据只包括节点树，如果传<code>true</code>则会包含主题、布局、视图等数据
    </p>
    ...
    <p>
      <code>type</code
      >：要导出的类型，可选值：png、svg、json、pdf（v0.2.1+）、smm（本质也是json）
    </p>
    <p>
      <code>isDownload</code>：是否需要直接触发下载，布尔值，默认为<code
        >false</code
      >
    </p>
    <p>
      <code>fileName</code>：（v0.1.6+）导出文件的名称，默认为<code
        >思维导图</code
      >
    </p>
    <p>如果是导出为<code>png</code>，那么可以传递第四个参数：</p>
    <p>
      <code>transparent</code>：v0.5.7+,
      <code>Boolean</code
      >，默认为<code>false</code>，指定导出图片的背景是否是透明的
    </p>
    <p>如果是导出为<code>svg</code>，那么可以传递第四个参数：</p>
    <p>
      <code>plusCssText</code
      >：附加的<code>css</code>样式，如果<code>svg</code>中存在<code>dom</code>节点，想要设置一些针对节点的样式可以通过这个参数传入
    </p>
    <p>
      如果是导出为<code>json</code>或<code>smm</code>，那么可以传递第四个参数：
    </p>
    <p>
      <code>withConfig</code
      >：<code>Boolean</code>，默认为<code>true</code>，指定导出的数据中是否包含配置数据，否则只导出纯节点树数据
    </p>
    <h3>toPos(x, y)</h3>
    <blockquote>
      <p>v0.1.5+</p>
    ...
    <p>
      注册插件，使用<code>MindMap.usePlugin</code>注册插件只能在实例化之前，实例化后注册的插件是不会生效的，所以如果想在实例化后注册插件可以使用实例的<code>addPlugin</code>方法。
    </p>
    ...
    <p>
      移除注册的插件，无论是通过<code>usePlugin</code>还是<code>addPlugin</code>方法注册的插件都可以移除。
    </p>
  </div>
</template>
