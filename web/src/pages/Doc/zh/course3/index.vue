<template>
  <div>
    ...
    <pre
      class="hljs"
    ><code>mindMap.execCommand(<span class="hljs-string">&#x27;INSERT_CHILD_NODE&#x27;</span>)
</code></pre>
    <p>
...
mindMap.execCommand(<span class="hljs-string">&#x27;INSERT_CHILD_NODE&#x27;</span>, <span class="hljs-literal">false</span>, [], {
    ...
    <p>
      2.然后在<code>node_tree_render_end</code>事件里通过该<code>id</code>来获取实例：
    </p>
    <pre
      class="hljs"
    ><code>mindMap.on(<span class="hljs-string">&#x27;node_tree_render_end&#x27;</span>, <span class="hljs-function">() =&gt;</span> {
    ...
    <h2>插入多个子节点</h2>
    <p>
      如果你要同时插入多个子节点，那么可以执行<code>INSERT_MULTI_CHILD_NODE</code>命令：
    </p>
    <pre
      class="hljs"
    ><code>mindMap.execCommand(<span class="hljs-string">&#x27;INSERT_MULTI_CHILD_NODE&#x27;</span>, [], childList)
</code></pre>
    ...
    <pre
      class="hljs"
    ><code>mindMap.execCommand(<span class="hljs-string">&#x27;INSERT_NODE&#x27;</span>)
</code></pre>
    <h2>插入多个同级节点</h2>
    <p>插入多个同级节点可以执行<code>INSERT_MULTI_NODE</code>命令：</p>
    <pre
      class="hljs"
    ><code>mindMap.execCommand(<span class="hljs-string">&#x27;INSERT_NODE&#x27;</span>. [], nodeList)
</code></pre>
    ...
    <pre
      class="hljs"
    ><code>mindMap.execCommand(<span class="hljs-string">&#x27;INSERT_PARENT_NODE&#x27;</span>)
</code></pre>
    <h2>删除节点</h2>
    <p>删除节点也是执行命令：</p>
    <pre
      class="hljs"
    ><code>mindMap.execCommand(<span class="hljs-string">&#x27;REMOVE_NODE&#x27;</span>)
</code></pre>
    <p>会删除当前激活的所有节点。</p>
    <h2>仅删除当前节点</h2>
    <p>
      <code>REMOVE_CURRENT_NODE</code
      >命令可以仅删除激活的节点，子节点不会被删除。
    </p>
    <pre
      class="hljs"
    ><code>mindMap.execCommand(<span class="hljs-string">&#x27;REMOVE_CURRENT_NODE&#x27;</span>)
</code></pre>
    <h2>前进回退</h2>
    <p>
      首先需要监听<code>back_forward</code>事件，事件回调中可以获取当前的历史记录总数，以及当前所在的历史记录索引，那么就可以判断当前是否处于历史记录的最开始，还是最后，然后对前进回退按钮进行禁用。
    </p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">const</span> isStart = ref(<span class="hljs-literal">true</span>)
