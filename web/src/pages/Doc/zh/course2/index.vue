<template>
  <div>
    <h1>操作节点内容</h1>
    <p>
      目前支持在节点中插入<code>图片</code>、<code>图标</code>、<code>超链接</code>、<code>备注</code>、<code>标签</code>、<code>概要</code>、<code>关联线</code>，本节教程会介绍如何通过UI界面来完成这些内容的插入。
    </p>
    <h2>监听节点激活事件</h2>
    <p>
      首先我们要监听节点的激活事件，如果当前没有激活节点，那么对应的UI界面肯定是禁用状态。可以通过<code>node_active</code>事件来监听节点激活事件：
    </p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">const</span> activeNodes = shallowRef([])
mindMap.on(<span class="hljs-string">&#x27;node_active&#x27;</span>, <span class="hljs-function">(<span class="hljs-params">node, activeNodeList</span>) =&gt;</span> {
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">const</span> hasRoot = computed(<span class="hljs-function">() =&gt;</span> {
...
<span class="hljs-keyword">const</span> hasGeneralization = computed(<span class="hljs-function">() =&gt;</span> {
    ...
    <h2>插入图片</h2>
    <p>
      选择图片和上传图片的功能需要你自行开发，假设我们已经上传了一张图片，我们就可以遍历当前激活的节点，挨个调用节点的<code>setImage</code>方法设置图片：
    </p>
    <pre
      class="hljs"
    ><code>activeNodes.value.forEach(<span class="hljs-function">(<span class="hljs-params">node</span>) =&gt;</span> {
    node.setImage({
    ...
    <p>
      图片回显也很简单，可以使用节点的<code>getData</code>方法获取节点的图片：
    </p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">const</span> img = activeNode.getData(<span class="hljs-string">&#x27;image&#x27;</span>)
<span class="hljs-keyword">const</span> imgTitle = activeNode.getData(<span class="hljs-string">&#x27;imageTitle&#x27;</span>)
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">const</span> iconList = [<span class="hljs-string">&#x27;priority_1&#x27;</span>, <span class="hljs-string">&#x27;...&#x27;</span>]<span class="hljs-comment">// 选择的图标</span>
activeNodes.value.forEach(<span class="hljs-function"><span class="hljs-params">node</span> =&gt;</span> {
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">const</span> iconList = activeNode.getData(<span class="hljs-string">&#x27;icon&#x27;</span>) || []
</code></pre>
    <h2>插入超链接</h2>
    <p>
      对于超链接，你需要实现两个输入框，分别输入<code>url</code>和名称，然后插入到节点：
    </p>
    <pre
      class="hljs"
    ><code>activeNodes.value.forEach(<span class="hljs-function"><span class="hljs-params">node</span> =&gt;</span> {
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">const</span> url = activeNode.getData(<span class="hljs-string">&#x27;hyperlink&#x27;</span>)
<span class="hljs-keyword">const</span> name = activeNode.getData(<span class="hljs-string">&#x27;hyperlinkTitle&#x27;</span>)
    ...
    <h3>自定义显示备注</h3>
    <p>
      如果你想显示更丰富的备注内容，比如支持<code>markdown</code>，那么你可以在实例化<code>simple-mind</code>时插入<code>customNoteContentShow</code>选项。
    </p>
    <p>
      设置备注内容还是使用<code>setNote</code>方法，只不过现在插入的是<code>markdown</code>字符串，因为内置的备注弹窗只能处理纯文本内容，所以你需要自己来渲染<code>markdown</code>：
    </p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">const</span> mindMap = <span class="hljs-keyword">new</span> MindMap({
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">const</span> note = activeNode.getData(<span class="hljs-string">&#x27;note&#x27;</span>)
</code></pre>
    <h2>插入标签</h2>
    <p>
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">const</span> tagArr = activeNode.getData(<span class="hljs-string">&#x27;tag&#x27;</span>) || []
</code></pre>
    <h2>插入概要</h2>
    <p>插入概要可以不需要实现UI，直接调用插入概要的命令即可：</p>
    <pre
      class="hljs"
    ><code>mindMap.execCommand(<span class="hljs-string">&#x27;ADD_GENERALIZATION&#x27;</span>, data)
</code></pre>
    <p>
      会给当前激活的节点插入一个概要节点，第二个参数<code>data</code>可以不传，默认为：
    </p>
    <pre class="hljs"><code>{
