<template>
  <div>
    ...
    <h2>主题配置属性一览</h2>
    <p>
      此处的配置可能未及时更新，完整的主题配置可以参考：<a
        href="https://github.com/wanglin2/mind-map/blob/main/simple-mind/src/themes/default.js"
        >default.js</a
      >。
    </p>
    <p>
      如果某个属性的值你不知道如何传，可以去在线demo中设置一下，然后导出json或smm文件查看对应的值。也可以参考demo项目中的配置<a
        href="https://github.com/wanglin2/mind-map/blob/main/web/src/config/zh.js"
        >zh.js</a
      >。
    </p>
    <pre class="hljs"><code>{
  ...
  <span class="hljs-attr">generalization</span>: {
    ...
    <p>
      ...
      >内置了很多主题，可以通过如下方式获取到所有的内置主题列表：
    </p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> { themeList } <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;simple-mind/src/constants/constant&#x27;</span>
<span class="hljs-comment">// import { themeList } from &#x27;simple-mind/src/utils/constant&#x27; v0.6.0以下版本使用该路径</span>
</code></pre>
    <blockquote>
      <p>
    ...
    <p>可以在实例化<code>simple-mind</code>时指定使用的主题：</p>
    ...
    <pre
      class="hljs"
    ><code>mindMap.setTheme(<span class="hljs-string">&#x27;classic&#x27;</span>)
</code></pre>
    <p>如果要获取当前使用的主题名称可以使用：</p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">const</span> theme = mindMap.getTheme()
</code></pre>
    <h2>修改主题的部分配置</h2>
    <p>
      要修改主题的部分配置，可以在实例化时通过<code>themeConfig</code>选项设置：
    </p>
    <pre class="hljs"><code><span class="hljs-keyword">new</span> MindMap({
  <span class="hljs-attr">themeConfig</span>: {
    ...
    <p>
      具体的主题属性可以参考前面小节，最终渲染时，<code>themeConfig</code>配置会和当前主题配置进行合并。
    </p>
  ...
  <span class="hljs-comment">// ...</span>
    ...
    <p>
      注意，你传入的这个对象会替换掉配置中的<code>themeConfig</code>原有的值。
    </p>
    <h2>定义新主题</h2>
...
mindMap.setTheme(<span class="hljs-string">&#x27;主题名称&#x27;</span>)
    ...
    <p>
      一个主题其实就是一个普通的对象，完整配置可以参考<a
        href="https://github.com/wanglin2/mind-map/blob/main/simple-mind/src/themes/default.js"
        >默认主题</a
      >，<code>defineTheme</code>方法会把你传入的配置和默认配置做合并。大部分主题其实需要自定义的部分不是很多，一个典型的自定义主题配置可以参考<a
    ...
    <pre
      class="hljs"
    ><code>MindMap.defineTheme(<span class="hljs-string">&#x27;redSpirit&#x27;</span>, {
    <span class="hljs-comment">// 背景颜色</span>
