<template>
  <div>
    <h1>插入和扩展节点图标</h1>
    <h2>插入图标</h2>
    <p><code>simple-mind</code>内置了一些图标：</p>
    <img src="../../../../assets/img/docs/iconList.jpg" />
    <p>你可以通过如下方式获取：</p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> { nodeIconList } <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;simple-mind/src/svg/icons&#x27;</span>
</code></pre>
    <p>
      如果你使用的是<code>umd</code>格式的文件，可以通过如下方式获取（v0.5.8+）：
    </p>
    ...
    <pre class="hljs"><code>[
  {
    ...
    <p>
      你可以通过UI界面展示出来，要给一个节点设置图标，可以使用<code>node.setIcon([])</code>方法，要获取节点当前的图标，可以使用<code>node.getData('icon')</code>方法。
    </p>
    <p>
      一个图标由<code>type_name</code>构成，比如前面的图标要插入节点，那么图标的值为<code>priority_1</code>。
    </p>
    <p>
    ...
    <p>
      除了使用内置的图标外，你也可以扩展图标，实例化<code>simple-mind</code>时传入你要扩展的图标列表即可：
    </p>
