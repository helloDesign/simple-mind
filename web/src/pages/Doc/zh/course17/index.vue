<template>
  <div>
    ...
    <p>
      目前支持导出为<code>.smm</code>、<code>.json</code>、<code>.svg</code>、<code>.png</code>、<code>.pdf</code>、<code>.md</code>、<code>.xmind</code>文件。
    </p>
    <p>
      <code>.smm</code
      >是<code>simple-mind</code>自己定义的一种文件，其实就是<code>json</code>文件，换了一个扩展名而已。
    </p>
    ...
    <p>
      <code>isDownload</code
      >：传<code>true</code>会触发下载，<code>false</code>则不会，函数会返回导出文件的数据，<code>data:url</code>格式，你可以自行下载，（v0.9.2之前的版本，<code>pdf</code>不支持该参数，默认会直接下载）。
    </p>
    ...
    <pre
      class="hljs"
    ><code>mindMap.export(type, isDownload, fileName, withConfig)
</code></pre>
    <p>
      <code>withConfig</code
      >指定导出的数据中是否要包含节点数据外的配置数据，比如使用的布局、主题等，传<code>true</code>，导出的结构如下：
    </p>
    <pre class="hljs"><code>{
    layout,
    root,
    ...
    <p>
      如果传<code>false</code>，导出的数据只有<code>root</code>部分，也就是纯节点树。
    </p>
    <p>示例：</p>
    <pre
      class="hljs"
    ><code>mindMap.export(<span class="hljs-string">&#x27;smm&#x27;</span>, <span class="hljs-literal">true</span>, <span class="hljs-string">&#x27;文件名&#x27;</span>, <span class="hljs-literal">true</span>)
mindMap.export(<span class="hljs-string">&#x27;json&#x27;</span>, <span class="hljs-literal">true</span>, <span class="hljs-string">&#x27;文件名&#x27;</span>, <span class="hljs-literal">false</span>)
    ...
    <pre
      class="hljs"
    ><code>mindMap.export(<span class="hljs-string">&#x27;png&#x27;</span>, <span class="hljs-literal">true</span>, <span class="hljs-string">&#x27;文件名&#x27;</span>)
mindMap.export(<span class="hljs-string">&#x27;pdf&#x27;</span>, <span class="hljs-literal">true</span>, <span class="hljs-string">&#x27;文件名&#x27;</span>)
    ...
    <pre
      class="hljs"
    ><code>mindMap.export(type, isDownload, fileName, plusCssText = <span class="hljs-string">&#x27;&#x27;</span>)
</code></pre>
    <p>
      如果开启了节点富文本编辑，也就是<code>svg</code>中会存在节点的<code>html</code>结构，这就又存在一个问题，因为浏览器对每个元素默认会设置一些样式，影响最大的就是<code>margin</code>和<code>padding</code>，这就有可能会导致节点中的文字错位，所以可以通过<code>plusCssText</code>参数传入<code>css</code>样式：
    </p>
    <blockquote>
      <p>
        在v0.6.16+版本后，plusCssText参数已被删除，改为在实例化时通过<code>resetCss</code>配置传入。
      </p>
    </blockquote>
    ...
    <pre
      class="hljs"
    ><code>mindMap.export(<span class="hljs-string">&#x27;md&#x27;</span>, <span class="hljs-literal">true</span>, <span class="hljs-string">&#x27;文件名&#x27;</span>)
</code></pre>
    <h3>导出为xmind</h3>
    <blockquote>
      <p>v0.6.6+</p>
    ...
    <pre
      class="hljs"
    ><code>mindMap.export(<span class="hljs-string">&#x27;xmind&#x27;</span>, <span class="hljs-string">&#x27;文件名&#x27;</span>)
</code></pre>
    <h2>导入</h2>
    <p>
      目前支持从<code>.smm</code>、<code>.json</code>、<code>.xmind</code>、<code>.xlsx</code>、<code>.md</code>格式的文件导入。
    </p>
    <h3>导入smm、json</h3>
    <p>
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">let</span> data = <span class="hljs-built_in">JSON</span>.parse(<span class="hljs-string">&#x27;json数据&#x27;</span>)
...
mindMap.view.reset()
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> xmind <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;simple-mind/src/parse/xmind.js&#x27;</span>
</code></pre>
    <p>如果使用的是<code>umd</code>文件，可以这样获取：</p>
    <pre class="hljs"><code>MindMap.xmind
</code></pre>
    <p>
      如果你是通过<code>input type=file</code
      >等方式获取到的<code>File</code>文件对象，那么可以直接传递给<code>parseXmindFile</code>方法解析，注意返回的是一个<code>Promise</code>实例，会返回解析后的节点树数据，使用<code>setData</code>方法渲染到画布即可。
    </p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">let</span> data = <span class="hljs-keyword">await</span> xmind.parseXmindFile(file)
mindMap.setData(data)
</code></pre>
    <p>
      <code>.xmind</code
      >文件本质上是一个压缩包，改成<code>zip</code>后缀可以解压缩，里面存在一个<code>content.json</code>文件，如果你自己解析出了这个文件，那么可以把这个文件内容传递给这个<code>transformXmind</code>方法进行转换：
    </p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">let</span> data = <span class="hljs-keyword">await</span> xmind.transformXmind(fileContent)
mindMap.setData(data)
</code></pre>
    <p>
      另外如果导入的是<code>xmind8</code>版本的数据，需要使用<code>transformOldXmind</code>方法。
    </p>
    <h3>导入xlsx</h3>
    <p>
      这个文件的导入没有内置方法，需要你自己开发，以下是一个使用<code>xlsx</code>库的方式：
    </p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> { read, utils } <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;xlsx&#x27;</span>
