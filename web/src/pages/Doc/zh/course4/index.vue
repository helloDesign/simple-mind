<template>
  <div>
    ...
    <p>
      样式总体上分为两类，一是常态的样式，二是激活的样式。设置的方法都是<code>setStyle</code>方法，通过第三个参数进行指定：
    </p>
...
node.setStyle(<span class="hljs-string">&#x27;样式属性&#x27;</span>, <span class="hljs-string">&#x27;样式值&#x27;</span>, <span class="hljs-literal">true</span>)
    ...
    <h3>设置文字样式</h3>
    <p>
      文字样式目前支持：<code>字体</code>、<code>字号</code>、<code>行高</code>、<code>颜色</code>、<code>加粗</code>、<code>斜体</code>、<code>划线</code>。
    </p>
    <p>
      这些样式选择的UI界面都需要你自行开发，然后调用节点的<code>setStyle</code>方法更新。
    </p>
...
activeNodes.value.forEach(<span class="hljs-function"><span class="hljs-params">node</span> =&gt;</span> {
    ...
    <h3>设置边框样式</h3>
    <p>
      边框样式支持设置：<code>颜色</code>、<code>虚线</code>、<code>线宽</code>、<code>圆角</code>。
    </p>
...
activeNodes.value.forEach(<span class="hljs-function"><span class="hljs-params">node</span> =&gt;</span> {
  ...
  },
  {
    ...
    <h3>设置线条样式</h3>
    <p>
      节点线条支持设置：<code>颜色</code>、<code>虚线</code>、<code>线宽</code>。
    </p>
...
activeNodes.value.forEach(<span class="hljs-function"><span class="hljs-params">node</span> =&gt;</span> {
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> { supportActiveStyle } <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;simple-mind/src/themes/default&#x27;</span>
...
<span class="hljs-keyword">const</span> checkIsSupportActive = <span class="hljs-function">(<span class="hljs-params">prop</span>) =&gt;</span> {
    ...
    <p>
      其他和数值常态样式是一样的，只需要给<code>setStyle</code>方法传入第三个参数：
    </p>
    <pre class="hljs"><code><span class="hljs-comment">// 设置边框颜色</span>
activeNodes.value.forEach(<span class="hljs-function"><span class="hljs-params">node</span> =&gt;</span> {
