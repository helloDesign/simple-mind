<template>
  <div>
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-class"><span class="hljs-keyword">class</span> <span class="hljs-title">YourPlugin</span> </span>{
...
Scrollbar.instanceName = <span class="hljs-string">&#x27;yourPlugin&#x27;</span>
    ...
    <p>
      需要给插件类添加一个静态属性<code>instanceName</code>，会将你的插件实例通过该属性保存到思维导图实例上，外部或其他插件想要获取你的插件实例时都需要通过该属性：
    </p>
    <pre class="hljs"><code>mindMap.yourPlugin.xxx
</code></pre>
    <p>插件存在两个生命周期函数：</p>
    <p>
      <code>beforePluginRemove</code
      >生命周期会在思维导图实例调用<code>removePlugin</code>方法时调用，代表思维导图实例并没有销毁，只是移除该插件。
    </p>
    <p>
      <code>beforePluginDestroy</code
      >生命周期会在销毁思维导图时调用，此时思维导图实例也会被销毁。
    </p>
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> ScrollbarPlugin <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;simple-mind/src/plugins/Scrollbar.js&#x27;</span>
...
Scrollbar.instanceName = <span class="hljs-string">&#x27;yourPlugin&#x27;</span>
    ...
    <p>
      当你完成了一个插件后，你可以考虑发布到<code>npm</code>，提供给其他开发者使用。
    </p>
  </div>
</template>
