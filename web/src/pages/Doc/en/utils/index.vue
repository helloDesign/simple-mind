<template>
  <div>
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> {walk, ...} <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;simple-mind/src/utils&#x27;</span>
</code></pre>
    <h3>Methods</h3>
    ...
    <p>
      Scale the image proportionally. Zoom to the specified size of
      <code>newWidth</code> and <code>newHeight</code> while maintaining the
      original aspect ratio of the image.
    </p>
    <h4>
      walk(root, parent, beforeCallback, afterCallback, isRoot, layerIndex = 0,
      index = 0)
    </h4>
    ...
    <p>
      <code>beforeCallback</code>: preorder traversal callback function,
      callback parameters are: root, parent, isRoot, layerIndex, index
    </p>
    <p>
      <code>afterCallback</code>: postorder traversal callback function,
      callback parameters are: root, parent, isRoot, layerIndex, index
    </p>
    ...
    <pre
      class="hljs"
    ><code>walk(tree, <span class="hljs-literal">null</span>, <span class="hljs-function">() =&gt;</span> {}, <span class="hljs-function">() =&gt;</span> {}, <span class="hljs-literal">false</span>, <span class="hljs-number">0</span>, <span class="hljs-number">0</span>);
</code></pre>
    ...
    <p>
      <code>maxWidth</code> and <code>maxHeight</code> can both be passed, or
      only one of them can be passed
    </p>
    <h4>resizeImg(imgUrl, maxWidth, maxHeight)</h4>
    <p>
      Resize image, internally loads the image first, then calls the
      <code>resizeImgSize</code> method, and returns a <code>promise</code>
    </p>
    <h4>simpleDeepClone(data)</h4>
    <p>
      Extremely simple deep copy method, can only be used for objects that are
      all basic data, otherwise it will throw an error
    </p>
    <h4>copyRenderTree(tree, root)</h4>
    <p>Copy render tree data, example:</p>
    <pre
      class="hljs"
    ><code>copyRenderTree({}, <span class="hljs-built_in">this</span>.mindMap.renderer.renderTree);
</code></pre>
    <h4>copyNodeTree(tree, root, removeActiveState, keepId)</h4>
    <ul>
      <li>
        <p>
          <code>removeActiveState</code>: <code>Boolean</code>, default is
          <code>false</code>, Whether to remove the active state of the node
        </p>
      </li>
      <li>
        <p>
          <code>removeId</code>：v0.7.3-fix.1+, Is remove the uid from the node
          data, default is <code>true</code>
        </p>
      </li>
    </ul>
    <blockquote>
      <ul>
        <li>
          <code>keepId</code>: (Original fourth parameter) v0.4.6+,
          <code>Boolean</code>, default is <code>false</code>, Whether to retain
          the <code>id</code> of the replicated node will be deleted by default
          to prevent duplicate node <code>id</code>. However, for mobile node
          scenarios, the original <code>id</code> of the node needs to be
          retained
        </li>
      </ul>
    </blockquote>
    <p>
      Copy node tree data, mainly eliminating the reference
      <code>node</code> instance <code>_node</code> and copying the
      <code>data</code> of the data object, example:
    </p>
    ...
    <p>
      Run tasks in task list asynchronously, tasks are run synchronously without
      order
    </p>
    <h4>degToRad(deg)</h4>
    <blockquote>
    ...
    <p>
      Join the <code>font</code> attribute value of the <code>css</code> font
    </p>
  ...
  size<span class="hljs-comment">// { width, height } width and height of image</span>
    ...
    <p>
      Get the type of a data, such as <code>Boolean</code>、<code>Array</code>.
    </p>
    ...
    <p>
      ...
      <code>&lt;p&gt;&lt;span&gt;&lt;/span&gt;&lt;p&gt;</code> into text wrapped
      in <code>\n</code>.
    </p>
    ...
    <p>
      Convert the wrapped text of <code>&lt;br&gt;</code> into node rich text
      content in the form of
      <code>&lt;p&gt;&lt;span&gt;&lt;/span&gt;&lt;p&gt;</code>.
    </p>
    <h4>isMobile()</h4>
    <blockquote>
      <p>v0.6.13+</p>
    ...
    <h4>
      checkTwoRectIsOverlap(minx1, maxx1, miny1, maxy1, minx2, maxx2, miny2,
      maxy2)
    </h4>
    <blockquote>
      <p>v0.7.2+</p>
    ...
    <ul>
      <li>
        <code>el</code>: DOM nodes, elements that can be focused, typically
        input box elements.
      </li>
    </ul>
    ...
    <ul>
      ...
      <li>
        <p>
          <code>data</code>：The data to be attached to all nodes in the
          specified node instance list tree.
        </p>
      </li>
    </ul>
    <p>
      Adding additional data to the specified node list tree data will modify
      the original data.
    </p>
    ...
    <ul>
      ...
      <li>
        <p>
          <code>createNewId</code>：v0.7.3-fix.1+, <code>Boolean</code>, default
          is <code>false</code>, If the node does not have a 'uid', a new 'uid'
          will be created. If 'true' is passed, a new 'uid' will be created
          regardless of whether the node data originally exists or not`
        </p>
      </li>
    </ul>
    <p>
      Adding a uid to the specified node list tree data (if the uid does not
      exist) will modify the original data.
    </p>
    ...
    <ul>
      <li>
        <code>list</code>：The array of node icons to be merged into the
        library.
      </li>
    </ul>
...
<span class="hljs-comment">// ]</span>
    ...
    <p>
      Escape the incoming string, currently escaping the following three
      characters:
    </p>
    <pre class="hljs"><code>&amp; -&gt; &amp;amp;
&lt; -&gt; &amp;lt;
&gt; -&gt; &amp;gt;
    ...
    <p>
      Generate colors based on incoming content, and the same content will
      generate the same color.
    </p>
    ...
    <ul>
      <li>
        <code>a</code>、<code>b</code>: Object | Array, Two objects to compare
      </li>
    </ul>
    <p>
      Determine whether two objects are the same, only handling objects or
      arrays.
    </p>
  ...
  img
    ...
    <p>
      ...
      <code>nodeData.children</code> list.
    </p>
    ...
    <p>
      Determine whether there is a hierarchical relationship from the given node
      instance list.
    </p>
    ...
    <p>
      Get the current version of the Chrome kernel used by the browser. If the
      current browser is not using the 'Chrome' kernel, an empty string will be
      returned.
    </p>
    ...
    <p>
      Convert flat objects into a tree structure. Reverse operation of the
      transformTreeDataToObject method.
    </p>
    ...
    <ul>
      ...
      <li>
        <p>
          <code>selector</code>：Node selectors, such as class selectors and id
          selectors
        </p>
      </li>
    </ul>
    <p>
      Remove the node of the specified selector from the specified HTML string,
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> drawBackgroundImageToCanvas <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;simple-mind/src/utils/simulateCSSBackgroundInCanvas&#x27;</span>
</code></pre>
    <p>Usage：</p>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">let</span> width = <span class="hljs-number">500</span>
...
drawBackgroundImageToCanvas(ctx, width, height, img, {
  ...
  <span class="hljs-keyword">if</span> (err) {
    ...
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">import</span> Lru <span class="hljs-keyword">from</span> <span class="hljs-string">&#x27;simple-mind/src/utils/Lru.js&#x27;</span>
</code></pre>
    <h3>Constructor</h3>
    <pre
      class="hljs"
    ><code><span class="hljs-keyword">let</span> lru = <span class="hljs-keyword">new</span> Lru(max)
</code></pre>
